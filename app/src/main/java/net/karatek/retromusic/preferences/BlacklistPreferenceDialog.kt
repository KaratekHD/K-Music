...
import net.karatek.retromusic.dialogs.BlacklistFolderChooserDialog
import net.karatek.retromusic.providers.BlacklistStore
...
class BlacklistPreferenceDialog : DialogFragment(), BlacklistFolderChooserDialog.FolderCallback {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        return MaterialDialog(requireContext(), BottomSheet(LayoutMode.WRAP_CONTENT)).show {
            title(net.karatek.retromusic.R.string.blacklist)
            ...
            neutralButton(text = getString(R.string.clear_action)) {
                MaterialDialog(requireContext(), BottomSheet(LayoutMode.WRAP_CONTENT)).show {
                    title(net.karatek.retromusic.R.string.clear_blacklist)
                    message(net.karatek.retromusic.R.string.do_you_want_to_clear_the_blacklist)
                    cornerRadius(PreferenceUtil.getInstance(requireContext()).dialogCorner)
                    positiveButton(net.karatek.retromusic.R.string.clear_action) {
            ...
            listItems(items = paths, waitForPositiveButton = false) { _, _, text ->
                MaterialDialog(context, BottomSheet(LayoutMode.WRAP_CONTENT)).show {
                    cornerRadius(PreferenceUtil.getInstance(requireContext()).dialogCorner)
                    title(net.karatek.retromusic.R.string.remove_from_blacklist)
                    message(text = Html.fromHtml(getString(net.karatek.retromusic.R.string.do_you_want_to_remove_from_the_blacklist, text)))
                    positiveButton(net.karatek.retromusic.R.string.remove_action) {
